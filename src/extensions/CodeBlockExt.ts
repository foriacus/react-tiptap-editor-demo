...
export const getLanguageByValueOrAlias = (
  ...
  const language = languages.find(
    (language) =>
      language.value === v || (language.alias && language.alias.includes(v))
  );
  return language!;
};
...
export const CodeBlockExt =
  CodeBlockLowlight.extend<MyCodeBlockLowlightOptions>({
    addOptions() {
      return {
        ...this.parent?.(),
        lowlight: {},
        ...
        addCodeExplain:
          ...
          ({ editor }) => {
            ...
            if (nodeSize <= pos + node.nodeSize + 2) {
              ...
              dispatch(
                tr.setSelection(
                  TextSelection.create(editor.state.doc, nodeSize - 2)
                )
              );
            } else {
              dispatch(
      ...
      return (props) => {
        ...
        if (!this.editor.isEditable) {
          container.innerHTML = `
                <div class="aie-codeblock-tools" contenteditable="false">
                    <div class="aie-codeblock-tools-lang" contenteditable="false" style="margin-right: 10px">${
                ...
                <div class="aie-codeblock-tools" contenteditable="false">
                    ${
                      this.options.codeCommentsAi
                        ? '<div class="aie-codeblock-tools-comments">自动注释</div>'
                        : ""
                    }
                    ${
                      this.options.codeExplainAi
                        ? '<div class="aie-codeblock-tools-explain">代码解释</div>'
                        : ""
                    }
                    <div class="aie-codeblock-tools-lang" contenteditable="false"><span>${
                      language || this.options.defaultLanguage
        ...
        const createEL = () => {
          ...
          div.addEventListener("click", (event) => {
            ...
            ).closest(".aie-codeblock-langs-item")!;
            if (target) {
        ...
        const instance = tippy(
          container.querySelector(".aie-codeblock-tools-lang")!,
          {
            content: createEL(),
        ...
        container
          .querySelector(".aie-codeblock-tools-comments")
          ?.addEventListener("click", (e: any) => {
            ...
            const clodeBlock = props.editor.state.doc.resolve(
              coordinates!.pos
            ).parent;
        ...
        container
          .querySelector(".aie-codeblock-tools-explain")
          ?.addEventListener("click", (e: any) => {
            // can not use the props node and its getPos, because node's attr and pos will change if dispatch transaction
