  ...
  addCommands() {
    return {
      ...
      uploadImage: (file: File) => () => {
        if (
          this.options.uploaderEvent &&
          this.options.uploaderEvent.onUploadBefore
        ) {
          if (
            this.options.uploaderEvent.onUploadBefore(
              file,
        ...
        const uploader =
          this.options.uploader || getUploader(this.options.uploadUrl!);
        uploader(
          file,
          this.options.uploadUrl!,
          this.options.uploadHeaders,
          ...
          .then((json) => {
            ...
            if (
              this.options.uploaderEvent &&
              this.options.uploaderEvent.onSuccess
            ) {
            ...
            if (json.errorCode === 0 && json.data && json.data.src) {
              const decorations = key.getState(
                this.editor.state
              ) as DecorationSet;
              let found = decorations.find(
                void 0,
                void 0,
                (spec) => spec.id == id
              );
              ...
              if (
                this.options.uploaderEvent &&
                this.options.uploaderEvent.onFailed
              ) {
          ...
          .catch((err) => {
            ...
            if (
              this.options.uploaderEvent &&
              this.options.uploaderEvent.onError
            ) {
  ...
  addNodeView() {
    return (e) => {
      ...
      container.innerHTML = `
                ...
                `;
      resize(container, e.editor.view.dom, (attrs) =>
        e.editor.commands.updateAttributes("image", attrs)
      );
          ...
          apply: (tr, set) => {
            ...
            if (action) {
              ...
              // remove decoration
                ...
                set = set.remove(
                  set.find(void 0, void 0, (spec) => spec.id == action.id)
                );
              }
            }
            return set;
        ...
        props: {
          ...
          handleDOMEvents: {
            drop(view, event) {
              ...
              dispatch(
                tr
                  .setSelection(TextSelection.create(doc, coordinates!.pos))
                  .scrollIntoView()
              );
          ...
          transformPastedHTML(html) {
            ...
            if (workspace?.children) {
              const imgNodes =
                document.documentElement.querySelectorAll("p > img");
