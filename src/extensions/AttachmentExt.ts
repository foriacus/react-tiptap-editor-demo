...
import { UploaderEvent } from "../core/Editor.ts";
            ...
            if (json.errorCode === 0 && json.data && json.data.href) {
              ...
              view.dispatch(
                view.state.tr
                  ...
                  .addMark(
                    found[0].from + 1,
                    fileName.length + found[0].from + 1,
        ...
        props: {
          ...
          handleDOMEvents: {
            drop(_, event) {
              ...
              const attachments = Array.from(event.dataTransfer.files).filter(
                (file) => !/video/i.test(file.type) && !/image/i.test(file.type)
              );
