  ...
  renderTemplate() {
    this.template = `
         ...
         <div style="display: flex" id="tippy">
             <span style="line-height: 18px;font-size: 14px;text-align:center;overflow: hidden; width: ${
               this.menuTextWidth
             }" id="text">
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="none" d="M0 0h24v24H0z"></path><path d="M12 14L8 10H16L12 14Z"></path></svg>
  ...
  createMenuElement() {
    ...
    for (let i = 0; i < this.menuData.length; i++) {
      ...
      item.innerHTML = `
            <div class="red-dot-container" id="item${i}"><div class="${
        i == 0 ? "red-dot" : ""
      }"></div></div>
  ...
  onTransaction(event: EditorEvents["transaction"]) {
    ...
    this.tippyEl
      ?.querySelector(`#item${activeIndex}`)!
      .children[0].classList.add("red-dot");
    if (this.refreshMenuText && this.textEl) {
      const menuTextResult = this.onMenuTextRender(activeIndex);
      if (
        typeof menuTextResult === "string" ||
        typeof menuTextResult === "number"
      ) {
