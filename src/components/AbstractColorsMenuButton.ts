...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill="none" d="M0 0h24v24H0z"></path><path d="M12 14L8 10H16L12 14Z"></path></svg>
    ...
    this.querySelector(".currentColor")!.addEventListener("click", () => {
      this.onColorItemClick!(
        this.historyColors.length > 0 ? this.historyColors[0] : "#ccc"
      );
    });
  ...
  createMenuElement() {
    ...
    div.innerHTML = `
        <div class="color-panel">
            <div class="color-panel-default-button" id="defaultColor">${t(
              "default"
            )}</div>
            <div style="display: flex;flex-wrap: wrap;padding-top: 5px">
                ${colors
                  .map((color, index) => {
                    return `<div class="color-item" data-color="#${color}" style="width: 18px;height:18px;margin:1px;padding:1px;border:1px solid #${
                      index == 0 ? "efefef" : color
    ...
    div.querySelector("#history-colors")!.addEventListener("click", (e) => {
      const target: HTMLDivElement = (e.target as any).closest(
        ".history-color-item"
  ...
  invokeColorItemClick(rootDiv: HTMLDivElement, item: HTMLDivElement) {
    ...
    localStorage.setItem(
      this.historyColorsKey,
      JSON.stringify(this.historyColors)
    );
