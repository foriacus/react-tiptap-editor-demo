...
export abstract class AbstractBubbleMenu
  extends HTMLElement
  implements AiEditorEvent
{
  editor?: Editor;
  ...
  connectedCallback() {
    this.innerHTML = `
            <div class="aie-bubble-menu">
               ${this.items!.map((item) => {
                 return `<div class="aie-bubble-menu-item ${
                   this.isActive(item.id) ? "active" : ""
                 }" id="${item.id}">${item.content}</div>`;
               }).join("")}
            </div>
        `;
