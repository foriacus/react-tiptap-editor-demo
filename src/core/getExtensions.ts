...
import { AiEditor, AiEditorOptions } from "./Editor.ts";
...
export const getExtensions = (
...

  {
    ...
    ret.push(
      ...
      ImageExt.configure({
        allowBase64:
          typeof options.image?.allowBase64 === "undefined"
            ? true
            : options.image?.allowBase64,
      ...
      Link.configure({
        openOnClick: false,
        autolink:
          typeof options.link?.autolink === "undefined"
            ? true
            : options.link?.autolink,
        HTMLAttributes: {
      ...
      CodeBlockExt.configure({
        ...
        codeExplainAi: options.ai?.codeBlock?.codeExplain || {
          model: "spark",
          prompt:
            "帮我对这个代码进行解释，返回代码的解释内容，注意，不需要对代码的注释进行解释",
        },
        codeCommentsAi: options.ai?.codeBlock?.codeComments || {
          model: "spark",
          prompt:
            "帮我对这个代码添加一些注释，并返回添加注释的代码，只返回代码",
        },
      }),
      VideoExt.configure({
