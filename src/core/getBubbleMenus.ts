...
import { AiEditor } from "./Editor.ts";
import {
  BubbleMenuOptions,
  BubbleMenuPlugin,
...
const createTextSelectionBubbleMenu = (aiEditor: AiEditor) => {
  const menuEl = document.createElement(
    "aie-bubble-text"
  ) as TextSelectionBubbleMenu;
  ...
  return createBubbleMenu("tableBubble", {
    ...
    tippyOptions: {
      ...
      getReferenceClientRect: () => {
        ...
        const domRect = posToDOMRect(view, from, to);
        const tablePos = aiEditor.innerEditor.state.selection.$from.posAtIndex(
          0,
          1
        );
