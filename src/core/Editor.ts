...
export class AiEditor {
  ...
  private initInnerEditor() {
    const rootEl =
      typeof this.options.element === "string"
        ? (document.querySelector(this.options.element) as Element)
    ...
    if (this.options.contentRetention && this.options.contentRetentionKey) {
      const cacheContent = localStorage.getItem(
        this.options.contentRetentionKey
      );
      if (cacheContent) {
  ...
  private onCreate(props: EditorEvents["create"]) {
    ...
    const _header =
      this.container.querySelector(".aie-container-header") || this.container;
    _header.appendChild(this.header);

    const _main =
      this.container.querySelector(".aie-container-main") || this.container;
    _main.appendChild(this.mainEl);

    const _footer =
      this.container.querySelector(".aie-container-footer") || this.container;
    ...
    if (
      ...
      if ("<p></p>" === html || "" === html) {
        ...
        localStorage.setItem(
          this.options.contentRetentionKey,
          JSON.stringify(this.innerEditor.getJSON())
        );
      }
    }
  }
  ...
  getSelectedText() {
    ...
    return getTextBetween(this.innerEditor.state.doc, {
      from: selection.from,
      to: selection.to,
    });
  }
